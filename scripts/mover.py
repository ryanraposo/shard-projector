...
class Mover(ThemedTk):
    """A Tkinter development tool that allows moving widgets and components during runtime. 

    Displays relative coordinates of the targeted widget. Allows formatted, code-ready 
    exporting to clipboard for easy use with Tkinter's 'place' geometry manager."""

    def __init__(
        self,
        target_widget: ttk.Widget,
        target_parent: ttk.Widget,
        widget_name: str,
        increment=1,
    ):
        ...
        self.btn_info_to_clipboard = ttk.Button(
            self, text="Copy", command=self.copy_to_clipboard
        )
        ...
        self.lbl_info.grid(row=2, column=0)

        self.btn_info_to_clipboard.grid(row=2, column=1)
    ...
    def _update_location(self):
        ...
        self.lbl_info.config(
            text="(x=" + str(self.widget_x) + ", y=" + str(self.widget_y) + ")"
        )
