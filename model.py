    ...
    def set_from_dict(self, dictionary):
        ...
        for section in dictionary:
            for k, v in dictionary[section].items():
                self.target.set(section, k, v)
        with open(self.path, 'w') as configfile:
            self.target.write(configfile)
...
class Shard:
    ...
    def __init__(self, path, cluster_name):
        ...
        self.config = ConfigurationParser(os.path.join(self.path, "server.ini"))
...
class ServerControl:
    ...
    def initialize_ui(self):
        ...
        # self.action_settings.configure(state=tk.DISABLED)
        # self.action_tasks.configure(state=tk.DISABLED)
